...
name: string;
suitable: {pregnancy: boolean; breastfeeding: boolean; postpartum: boolean; baby: boolean};
...
const mockPhotoUrl = "https://public.readdy.ai/ai/img_res/15aafc7aac3329f5357757426d378ef5.jpg";
setFoodImage(mockPhotoUrl);
...
nutrients: [
{ name: "Omega-3 Fatty Acids", amount: "1.5", unit: "g" },
{ name: "Protein", amount: "32", unit: "g" },
{ name: "Iron", amount: "2.5", unit: "mg" },
{ name: "Calcium", amount: "120", unit: "mg" },
{ name: "Vitamin D", amount: "11", unit: "μg" },
{ name: "Folate", amount: "83", unit: "μg" },
{ name: "Fiber", amount: "6", unit: "g" },
{ name: "Vitamin B12", amount: "4.8", unit: "μg" },
...
fats: 18,
carbs: 35,
recommendation: "Excellent choice for pregnancy and breastfeeding. The omega-3 fatty acids in salmon support baby's brain development. The iron and protein help with postpartum recovery. Not suitable for babies under 6 months. For babies 6+ months, the salmon and vegetables can be pureed separately."
});
}, 2000);
};
...
let response = '';
const userMsg = userMessage.text.toLowerCase();
if (userMsg.includes('sleep') || userMsg.includes('tired')) {
response = "Sleep deprivation is common for new mothers. Try to sleep when your baby sleeps, even during the day. Consider asking your partner or a family member to take a night feeding so you can get a longer stretch of sleep. Creating a relaxing bedtime routine might also help.";
} else if (userMsg.includes('overwhelm') || userMsg.includes('stress') || userMsg.includes('anxious')) {
response = "It's completely normal to feel overwhelmed as a new mother. Remember that it's okay to ask for help. Try to take small breaks throughout the day, even just 5 minutes of deep breathing can help. If these feelings persist or worsen, please consider speaking with your healthcare provider about postpartum anxiety or depression.";
} else if (userMsg.includes('breastfeed') || userMsg.includes('nursing') || userMsg.includes('nipple')) {
response = "For sore nipples while breastfeeding, try applying expressed breast milk or lanolin after feeding. Ensure proper latch - baby's mouth should cover most of the areola, not just the nipple. Different feeding positions can also help reduce soreness. If pain persists, consider consulting with a lactation consultant.";
} else if (userMsg.includes('self-care') || userMsg.includes('time for myself')) {
response = "Quick self-care ideas: take a 10-minute shower with a nice scented soap, enjoy a cup of tea while it's still hot, listen to your favorite song, or step outside for fresh air. Remember, self-care isn't selfish - it helps you be a better mom by recharging your energy.";
} else {
response = "Thank you for sharing. As a new mother, it's important to address both your physical and emotional needs. Would you like some specific advice about postpartum recovery, breastfeeding, sleep strategies, or emotional wellbeing?";
}
...
<p className="font-medium">Cry AI</p>
<p className="text-xs text-gray-500 mt-1">Translator</p>
</button>
<button
className="p-4 rounded-full shadow-md text-center cursor-pointer !rounded-button bg-white relative"
onClick={() => setShowFoodModal(true)}
>
<i className="fas fa-utensils text-xl text-blue-500 mb-2"></i>
<p className="font-medium">Food AI</p>
<p className="text-xs text-gray-500 mt-1">Analyzer</p>
...
<p className="text-xs text-gray-500 mt-1">Essentials</p>
</button>
</a>
</div>
{isFeeding && (
...
<div className="bg-white p-4 rounded-lg shadow-md mb-6">
<div className="flex justify-between items-center mb-3">
<h3 className="font-medium">Food Analyzer</h3>
<button
className="text-blue-500 text-sm cursor-pointer !rounded-button"
onClick={() => setShowFoodModal(true)}
>
...
<div>
<p className="font-semibold">Smart Nutrition Guide</p>
<p className="text-xs text-gray-500">Take a photo of your food to check if it's suitable for you and your baby</p>
</div>
</div>
<button
className="w-full mt-3 py-2 bg-blue-500 text-white rounded-full cursor-pointer !rounded-button flex items-center justify-center"
onClick={() => setShowFoodModal(true)}
>
...
<button
className="flex items-center text-gray-600 cursor-pointer !rounded-button"
onClick={() => {
...
voiceButton?.classList.remove('text-gray-600');
...
transcript: '',
timer: setTimeout(() => {
// Simulate getting some text after 3 seconds
const sampleTexts = [
"I'm feeling tired today but happy to see my baby's smile. The night feedings are tough but worth it.",
"Today was a good day. Emma slept for 4 hours straight which gave me time to rest too.",
"I'm worried about going back to work next month. Not sure how I'll manage everything."
];
const randomText = sampleTexts[Math.floor(Math.random() * sampleTexts.length)];
// Add the transcribed text to the journal
setJournalText(prev => prev + (prev ? ' ' : '') + randomText);
// Reset the recording state
voiceStatus?.classList.add('hidden');
voiceButton?.classList.remove('text-red-500');
voiceButton?.classList.add('text-gray-600');
}, 3000)
};
// Store the recognition object for later access
...
if (window.currentRecognition?.timer) {
clearTimeout(window.currentRecognition.timer);
}
}
}}
id="voiceButton"
...
<button
className="px-4 py-2 bg-blue-500 text-white rounded-full cursor-pointer !rounded-button flex items-center"
onClick={() => {
if (journalText.trim()) {
// Create a timestamp
const now = new Date();
...
const confirmationEl = document.getElementById('journalConfirmation');
if (confirmationEl) {
...
id="saveJournalButton"
>
Save
</button>
</div>
<div id="journalConfirmation" className="mt-3 bg-green-50 p-2 rounded-lg text-green-700 text-sm hidden">
<div className="flex items-center">
<i className="fas fa-check-circle mr-2"></i>
<span>Journal entry saved successfully on {new Date().toLocaleDateString()} at {new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
</div>
</div>
</div>
<div className="bg-white p-4 rounded-lg shadow-md mb-6">
<h3 className="font-medium mb-3">Talk to AI Wellness Coach</h3>
<div className="max-h-60 overflow-y-auto mb-3" id="chatContainer">
...
<div className={`inline-block p-3 rounded-lg ${message.sender === 'user' ? 'bg-blue-100 text-blue-800' : 'bg-gray-50'}`}>
<p className="text-sm">{message.text}</p>
</div>
{message.sender === 'user' && <p className="text-xs text-gray-500 mt-1">You</p>}
{message.sender === 'ai' && <p className="text-xs text-gray-500 mt-1">AI Wellness Coach</p>}
</div>
))}
{isTyping && (
<div className="mb-3">
...
/>
<button
className="bg-blue-500 text-white px-4 rounded-r-lg cursor-pointer !rounded-button"
...
<button
className="text-xs bg-blue-50 p-2 rounded-full text-blue-600 cursor-pointer !rounded-button"
onClick={() => handleQuickQuestion("I'm having trouble sleeping. Any tips?")}
>
Trouble sleeping?
</button>
<button
className="text-xs bg-blue-50 p-2 rounded-full text-blue-600 cursor-pointer !rounded-button"
onClick={() => handleQuickQuestion("I'm feeling overwhelmed with motherhood. What can I do?")}
>
Feeling overwhelmed
</button>
<button
className="text-xs bg-blue-50 p-2 rounded-full text-blue-600 cursor-pointer !rounded-button"
onClick={() => handleQuickQuestion("Can you share some breastfeeding tips for a sore nipple?")}
>
Breastfeeding tips
</button>
<button
className="text-xs bg-blue-50 p-2 rounded-full text-blue-600 cursor-pointer !rounded-button"
onClick={() => handleQuickQuestion("What are some quick self-care ideas for busy moms?")}
>
Self-care ideas
</button>
</div>
...
<button
className="w-full mt-3 py-2 bg-blue-500 text-white rounded-lg cursor-pointer !rounded-button"
onClick={() => document.getElementById('askQuestionModal')?.classList.remove('hidden')}
>
Ask a Question
</button>
</div>
...
{/* Ask Question Modal */}
<div id="askQuestionModal" className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 hidden">
<div className="bg-white rounded-lg w-11/12 max-w-md">
<div className="p-4 border-b border-gray-200">
<div className="flex justify-between items-center">
<h3 className="font-medium text-lg">Ask an Expert</h3>
<button
onClick={() => document.getElementById('askQuestionModal')?.classList.add('hidden')}
className="text-gray-500 hover:text-gray-700 !rounded-button"
>
<i className="fas fa-times"></i>
</button>
...
<div className="p-4">
<div className="mb-4">
<label className="block text-gray-700 text-sm font-medium mb-2">Question Title</label>
<input
type="text"
className="w-full p-2 border border-gray-300 rounded-lg"
placeholder="E.g., When should I start solid foods?"
/>
</div>
<div className="mb-4">
...
<option value="development">Development & Milestones</option>
<option value="behavior">Behavior</option>
<option value="other">Other</option>
</select>
</div>
<div className="mb-4">
<label className="block text-gray-700 text-sm font-medium mb-2">Question Details</label>
<textarea
...
<div className="mb-4">
<label className="block text-gray-700 text-sm font-medium mb-2">Attach Images (Optional)</label>
<div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
<button
className="w-full py-2 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center !rounded-button"
>
<i className="fas fa-camera mr-2"></i>
Add Photo
</button>
...
<p className="text-sm text-blue-700">Your question will be answered by verified healthcare professionals and experts.</p>
</div>
</div>
<div id="questionSubmitConfirmation" className="hidden bg-green-50 p-3 rounded-lg mb-4">
<div className="flex items-center">
<i className="fas fa-check-circle text-green-500 mr-2"></i>
<p className="text-sm text-green-700">Your question has been submitted! Experts will respond within 24-48 hours.</p>
</div>
</div>
<div className="flex space-x-2 mt-6">
<button
...
onClick={() => {
// Show confirmation message
document.getElementById('questionSubmitConfirmation')?.classList.remove('hidden');
// Hide confirmation and close modal after delay
setTimeout(() => {
document.getElementById('questionSubmitConfirmation')?.classList.add('hidden');
...
className="text-gray-500 hover:text-gray-700 mr-2 !rounded-button"
>
<i className="fas fa-arrow-left"></i>
</button>
<h3 className="font-medium text-lg">Food Analyzer</h3>
</div>
<button
...
<h4 className="font-medium">AI Food Analysis</h4>
<p className="text-sm text-gray-600 mt-1">Take a photo of your food to check if it's suitable for you and your baby</p>
</div>
<div className="mb-4">
<div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
{foodImage ? (
<div className="relative">
<img src={foodImage} alt="Food" className="max-h-48 mx-auto rounded-lg" />
<button
...
{foodImage && !isAnalyzingFood && (
<button
onClick={analyzeFood}
className="w-full py-2 bg-blue-500 text-white rounded-lg flex items-center justify-center !rounded-button"
>
...
<p className="text-blue-500">Analyzing your food...</p>
</div>
)}
</>
)}
...
<div>
<div className="mb-4">
<img src={foodImage} alt="Food" className="w-full h-48 object-cover rounded-lg" />
</div>
<div className="bg-blue-50 p-4 rounded-lg mb-4">
<h4 className="font-medium text-lg text-blue-800">{foodAnalysisResult.name}</h4>
<div className="flex justify-between items-center mt-2">
<div className="flex-1 text-center">
<p className="text-xs text-gray-500">Calories</p>
<p className="font-semibold">{foodAnalysisResult.calories}</p>
</div>
<div className="flex-1 text-center">
<p className="text-xs text-gray-500">Protein</p>
<p className="font-semibold">{foodAnalysisResult.protein}g</p>
</div>
<div className="flex-1 text-center">
<p className="text-xs text-gray-500">Fats</p>
<p className="font-semibold">{foodAnalysisResult.fats}g</p>
</div>
<div className="flex-1 text-center">
<p className="text-xs text-gray-500">Carbs</p>
<p className="font-semibold">{foodAnalysisResult.carbs}g</p>
</div>
</div>
</div>

<h5 className="font-medium mb-2">Key Nutrients</h5>
<div className="bg-gray-50 p-3 rounded-lg mb-4">
<div className="grid grid-cols-2 gap-y-2">
{foodAnalysisResult.nutrients.map((nutrient, index) => (
<div key={index} className="flex items-center">
<div className="w-2 h-2 rounded-full bg-blue-500 mr-2"></div>
<span className="text-sm">{nutrient.name}: {nutrient.amount}{nutrient.unit}</span>
</div>
))}
</div>
</div>

<h5 className="font-medium mb-2">Suitability</h5>

<div className="mb-4 space-y-3">
<div className={`p-3 rounded-lg ${foodAnalysisResult.suitable.pregnancy ? 'bg-green-50' : 'bg-red-50'}`}>
<div className="flex items-center mb-1">
<div className={`w-6 h-6 rounded-full ${foodAnalysisResult.suitable.pregnancy ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'} flex items-center justify-center mr-2`}>
<i className={`fas fa-${foodAnalysisResult.suitable.pregnancy ? 'check' : 'times'} text-sm`}></i>
</div>
<span className="font-medium">Pregnancy</span>
</div>
<p className="text-sm ml-8">
{foodAnalysisResult.suitable.pregnancy 
  ? "This food is safe during pregnancy. It contains essential nutrients that support fetal development and maternal health. The protein and omega-3 fatty acids are particularly beneficial for brain development."
  : "This food should be limited or avoided during pregnancy. It may contain ingredients that pose risks to fetal development or maternal health. Consider alternatives that provide similar nutrients without the risks."}
</p>
</div>

<div className={`p-3 rounded-lg ${foodAnalysisResult.suitable.breastfeeding ? 'bg-green-50' : 'bg-red-50'}`}>
<div className="flex items-center mb-1">
<div className={`w-6 h-6 rounded-full ${foodAnalysisResult.suitable.breastfeeding ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'} flex items-center justify-center mr-2`}>
<i className={`fas fa-${foodAnalysisResult.suitable.breastfeeding ? 'check' : 'times'} text-sm`}></i>
</div>
<span className="font-medium">Breastfeeding</span>
</div>
<p className="text-sm ml-8">
{foodAnalysisResult.suitable.breastfeeding 
  ? "This food is excellent for breastfeeding mothers. The nutrients can help maintain milk supply and quality. Some mothers report that foods like this may increase milk production. It provides energy needed for lactation without affecting milk flavor."
  : "This food may affect milk flavor or cause digestive discomfort in some babies. If you notice your baby becoming fussy after you consume this, consider eliminating it from your diet temporarily to see if it helps."}
</p>
</div>

<div className={`p-3 rounded-lg ${foodAnalysisResult.suitable.postpartum ? 'bg-green-50' : 'bg-red-50'}`}>
<div className="flex items-center mb-1">
<div className={`w-6 h-6 rounded-full ${foodAnalysisResult.suitable.postpartum ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'} flex items-center justify-center mr-2`}>
<i className={`fas fa-${foodAnalysisResult.suitable.postpartum ? 'check' : 'times'} text-sm`}></i>
</div>
<span className="font-medium">Postpartum</span>
</div>
<p className="text-sm ml-8">
{foodAnalysisResult.suitable.postpartum 
  ? "This food supports postpartum recovery. It contains nutrients that help with tissue repair, energy restoration, and overall healing. The iron and protein content can help address postpartum blood loss and muscle recovery."
  : "This food may not provide optimal nutrition for postpartum recovery. Focus instead on foods rich in iron, protein, and vitamins that support healing and energy restoration after childbirth."}
</p>
</div>

<div className={`p-3 rounded-lg ${foodAnalysisResult.suitable.baby ? 'bg-green-50' : 'bg-red-50'}`}>
<div className="flex items-center mb-1">
<div className={`w-6 h-6 rounded-full ${foodAnalysisResult.suitable.baby ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'} flex items-center justify-center mr-2`}>
<i className={`fas fa-${foodAnalysisResult.suitable.baby ? 'check' : 'times'} text-sm`}></i>
</div>
<span className="font-medium">Baby</span>
</div>
<p className="text-sm ml-8">
{foodAnalysisResult.suitable.baby 
  ? "This food is appropriate for babies over 6 months, when properly prepared. For babies 6-9 months, puree thoroughly. For older babies, you can mash or cut into appropriate sizes for their developmental stage. Introduce one ingredient at a time to monitor for allergies."
  : "This food is not recommended for babies under 12 months. It may pose choking hazards, contain allergens, or have ingredients that are difficult for babies to digest. Wait until your child is older before introducing this food."}
</p>
</div>
</div>
...
<p className="text-sm">{foodAnalysisResult.recommendation}</p>
</div>

<div className="bg-gray-50 p-3 rounded-lg mb-4">
<h5 className="font-medium mb-2 text-gray-700">Allergy Warning</h5>
<div className="flex items-center">
<i className="fas fa-exclamation-triangle text-yellow-500 mr-2"></i>
<p className="text-sm text-gray-600">Please be cautious of any allergies you and/or your family may have before consuming this food.</p>
</div>
</div>
...
<button
className="flex-1 py-2 bg-gray-200 rounded-lg !rounded-button"
onClick={() => setShowFoodModal(false)}
>
Close
</button>
